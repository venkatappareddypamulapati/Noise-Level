        ...
        for y_cord in range(patches_img.shape[1]):
            ...
            for width_pixel in range(width):
                for height_pixel in range(height):
                    if  np.abs(single_patch_img[width//2, height//2]-single_patch_img[width_pixel, height_pixel])<3*np.std(single_patch_img): # Gaussian Noise
...
for key, val in test_dict.items():
    if val not in temp:
        ...
        #print(key)"""
#test_dict = {4:1, 2:1,3:1, 3:2, 8:4, 5:4,4:4, 6:7}
...
slope, intercept, r_value, p_value, std_err = stats.linregress(value_counts, noise_std)

# Create the scatter plot
plt.scatter(value_counts, noise_std)

# Add the linear regression line to the plot
plt.plot(value_counts, intercept + slope*np.array(value_counts), 'r')
